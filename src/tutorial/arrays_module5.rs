...
pub fn arrays_module5_sliding_windows_and_prefix_sum_tricks() {
    ...
    let size_count = 18; // How many itterations to double from size_interval
...
fn range_sum_naive(arr: &[i32], l: usize, r: usize) -> i32 {
    let mut total = 0;
    for i in l..=r {
        total += arr[i];
    }
...
fn make_prefix_sum_array(arr: &[i32]) -> Vec<i32> {
    let size = arr.len();
    if size == 0 {
        return Vec::new();
    }

    let mut suffix_sum_array = Vec::with_capacity(size);
    suffix_sum_array.push(arr[0]);
    for i in 1..size {
        suffix_sum_array.push(suffix_sum_array[i - 1] + arr[i]);
    }
    suffix_sum_array
}
